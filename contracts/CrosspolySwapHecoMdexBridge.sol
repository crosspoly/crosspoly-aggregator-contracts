...
contract CrosspolySwapHecoMdexBridge is Ownable {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'CrosspolySwapHecoMdexBridge: EXPIRED');
        _;
    }
    ...
    constructor (
        ...
        require(_wht != address(0), "CrosspolySwapHecoMdexBridge: ZERO_WHT_ADDRESS");
        require(_factory != address(0), "CrosspolySwapHecoMdexBridge: ZERO_FACTORY_ADDRESS");
        require(_swapper != address(0), "CrosspolySwapHecoMdexBridge: ZERO_SWAPPER_ADDRESS");
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(amountOut >= amountOutMin, 'CrosspolySwapHecoMdexBridge: INSUFFICIENT_OUTPUT_AMOUNT');
        return amountOut;
    }
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        require(path[0] == WHT, 'CrosspolySwapHecoMdexBridge: INVALID_PATH');
        uint amountIn = msg.value.sub(fee);
        require(amountIn > 0, 'CrosspolySwapHecoMdexBridge: INSUFFICIENT_INPUT_AMOUNT');
        IWHT(WHT).deposit{value: amountIn}();
        assert(IWHT(WHT).transfer(MdexLibrary.pairFor(mdexFactory, path[0], path[1]), amountIn));
        uint balanceBefore = IHRC20(path[path.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IHRC20(path[path.length - 1]).balanceOf(address(this)).sub(balanceBefore);
        require(amountOut >= swapAmountOutMin, 'CrosspolySwapHecoMdexBridge: INSUFFICIENT_OUTPUT_AMOUNT');
        return amountOut;
    }
    ...
    function _swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(path[path.length - 1] == WHT, 'CrosspolySwapHecoMdexBridge: INVALID_PATH');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, MdexLibrary.pairFor(mdexFactory, path[0], path[1]), amountIn
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            require(IMdexFactory(mdexFactory).getPair(input, output) != address(0), "CrosspolySwapHecoMdexBridge: PAIR_NOT_EXIST");
