...
contract CrosspolySwapBSCPancakeBridge is Ownable {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'CrosspolySwapBSCPancakeBridge: EXPIRED');
        _;
    }
    ...
    constructor (
        ...
        require(_wbnb != address(0), "CrosspolySwapBSCPancakeBridge: ZERO_WBNB_ADDRESS");
        require(_factory != address(0), "CrosspolySwapBSCPancakeBridge: ZERO_FACTORY_ADDRESS");
        require(_swapper != address(0), "CrosspolySwapBSCPancakeBridge: ZERO_SWAPPER_ADDRESS");
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(amountOut >= amountOutMin, 'CrosspolySwapBSCPancakeBridge: INSUFFICIENT_OUTPUT_AMOUNT');
        return amountOut;
    }
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        require(path[0] == WBNB, 'CrosspolySwapBSCPancakeBridge: INVALID_PATH');
        uint amountIn = msg.value.sub(fee);
        require(amountIn > 0, 'CrosspolySwapBSCPancakeBridge: INSUFFICIENT_INPUT_AMOUNT');
        IWBNB(WBNB).deposit{value: amountIn}();
        assert(IWBNB(WBNB).transfer(PancakeLibrary.pairFor(pancakeFactory, path[0], path[1]), amountIn));
        uint balanceBefore = IBEP20(path[path.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IBEP20(path[path.length - 1]).balanceOf(address(this)).sub(balanceBefore);
        require(amountOut >= swapAmountOutMin, 'CrosspolySwapBSCPancakeBridge: INSUFFICIENT_OUTPUT_AMOUNT');
        return amountOut;
    }
    ...
    function _swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(path[path.length - 1] == WBNB, 'CrosspolySwapBSCPancakeBridge: INVALID_PATH');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(pancakeFactory, path[0], path[1]), amountIn
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            require(IPancakeFactory(pancakeFactory).getPair(input, output) != address(0), "CrosspolySwapBSCPancakeBridge: PAIR_NOT_EXIST");
