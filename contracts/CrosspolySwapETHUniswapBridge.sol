...
contract CrosspolySwapETHUniswapBridge is Ownable {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'CrosspolySwapETHUniswapBridge: EXPIRED');
        _;
    }
    ...
    constructor (
        ...
        require(_weth != address(0), "CrosspolySwapETHUniswapBridge: ZERO_WETH_ADDRESS");
        require(_factory != address(0), "CrosspolySwapETHUniswapBridge: ZERO_FACTORY_ADDRESS");
        require(_swapper != address(0), "CrosspolySwapETHUniswapBridge: ZERO_SWAPPER_ADDRESS");
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(amountOut >= amountOutMin, 'CrosspolySwapETHUniswapBridge: INSUFFICIENT_OUTPUT_AMOUNT');
        return amountOut;
    }
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        require(path[0] == WETH, 'CrosspolySwapETHUniswapBridge: INVALID_PATH');
        uint amountIn = msg.value.sub(fee);
        require(amountIn > 0, 'CrosspolySwapETHUniswapBridge: INSUFFICIENT_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amountIn}();
        assert(IWETH(WETH).transfer(UniswapV2Library.pairFor(uniswapFactory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IERC20(path[path.length - 1]).balanceOf(address(this)).sub(balanceBefore);
        require(amountOut >= swapAmountOutMin, 'CrosspolySwapETHUniswapBridge: INSUFFICIENT_OUTPUT_AMOUNT');
        return amountOut;
    }
    ...
    function _swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(path[path.length - 1] == WETH, 'CrosspolySwapETHUniswapBridge: INVALID_PATH');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(uniswapFactory, path[0], path[1]), amountIn
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            require(IUniswapV2Factory(uniswapFactory).getPair(input, output) != address(0), "CrosspolySwapETHUniswapBridge: PAIR_NOT_EXIST");
